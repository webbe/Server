...
// Converts Titanium Slot IDs to RoF2 Slot IDs for use in Encodes
static inline structs::ItemSlotStruct TitaniumToRoF2Slot(uint32 TitaniumSlot)
{
	structs::ItemSlotStruct RoF2Slot;
	RoF2Slot.SlotType = 0xffff;
	...
	{
		RoF2Slot.SlotType = 0;
		RoF2Slot.MainSlot = TitaniumSlot;
		if (TitaniumSlot == 9999)
		{
			RoF2Slot.MainSlot = 21;
		}
	...
	{
		RoF2Slot.SlotType = 5;
		RoF2Slot.MainSlot = TitaniumSlot - 31;
	}*/
	else if (TitaniumSlot > 250 && TitaniumSlot < 341)
	{
		RoF2Slot.SlotType = 0;
		TempSlot = TitaniumSlot - 1;
		RoF2Slot.MainSlot = int(TempSlot / 10) - 2;
		RoF2Slot.SubSlot = TempSlot - ((RoF2Slot.MainSlot + 2) * 10);
		if (RoF2Slot.MainSlot > 30)
		{
			RoF2Slot.MainSlot = 33;
		}
	}
	else if (TitaniumSlot > 399 && TitaniumSlot < 405)	// Tribute
	{
		RoF2Slot.SlotType = 6;
		RoF2Slot.MainSlot = TitaniumSlot - 400;
	}
	else if (TitaniumSlot > 1999 && TitaniumSlot < 2271)
	{
		...
		if (TempSlot > 30)
		{
			RoF2Slot.MainSlot = int(TempSlot / 10) - 3;
			RoF2Slot.SubSlot = TempSlot - ((RoF2Slot.MainSlot + 3) * 10);
		}
	}
	...
	_log(NET__ERROR, "Convert Titanium Slot %i to RoF2 Slots: Type %i, Unk2 %i, Main %i, Sub %i, Aug %i, Unk1 %i", TitaniumSlot, RoF2Slot.SlotType, RoF2Slot.Unknown02, RoF2Slot.MainSlot, RoF2Slot.SubSlot, RoF2Slot.AugSlot, RoF2Slot.Unknown01);

	return RoF2Slot;
}

static inline uint32 RoF2ToTitaniumSlot(structs::ItemSlotStruct RoF2Slot)
{
	uint32 TitaniumSlot = 0xffffffff;
	uint32 TempSlot = 0;

	if (RoF2Slot.SlotType == 0 && RoF2Slot.MainSlot < 57)	// Worn/Personal Inventory and Cursor (Originally 51)
	{
		if (RoF2Slot.MainSlot == 21)			// Power Source
		{
			TempSlot = 9999;
		}
		else if (RoF2Slot.MainSlot >= 33)	// Cursor and Extended Corpse Inventory
		{
			TempSlot = RoF2Slot.MainSlot - 3;
		}
		/*else if (RoF2Slot.MainSlot == 31 || RoF2Slot.MainSlot == 32) { // 9th and 10th RoF inventory/corpse slots
		...
		else if (RoF2Slot.MainSlot >= 22)	// Ammo and Main Inventory
		{
		...
		if (RoF2Slot.SubSlot >= 0)			// Bag Slots
		{
			TempSlot = ((TempSlot + 3) * 10) + RoF2Slot.SubSlot + 1;
		}

		TitaniumSlot = TempSlot;
	}
	else if (RoF2Slot.SlotType == 1)		// Bank Slots
	{
		TempSlot = 2000;
		if (RoF2Slot.SubSlot >= 0)
		{
			TempSlot += ((RoF2Slot.MainSlot + 3) * 10) + RoF2Slot.SubSlot + 1;
		}
		else
		{
			TempSlot += RoF2Slot.MainSlot;
	...
	else if (RoF2Slot.SlotType == 2)		// Shared Bank Slots
	{
		TempSlot = 2500;
		if (RoF2Slot.SubSlot >= 0)
		{
	...
	else if (RoF2Slot.SlotType == 3)		// Trade Slots
	{
		TempSlot = 3000;
		if (RoF2Slot.SubSlot >= 0)
		{
			TempSlot += 100 + (RoF2Slot.MainSlot * 10) + RoF2Slot.SubSlot;
		}
		else
		{
			TempSlot += RoF2Slot.MainSlot;
	...
	else if (RoF2Slot.SlotType == 4)		// Tradeskill Container Slots
	{
		TempSlot = 4000;
	...
	/*else if (RoF2Slot.SlotType == 5)		// Cursor Buffer
	{
		TempSlot = 31;
		if (RoF2Slot.MainSlot >= 0)
		{
	...
	_log(NET__ERROR, "Convert RoF2 Slots: Type %i, Unk2 %i, Main %i, Sub %i, Aug %i, Unk1 %i to Titanium Slot %i", RoF2Slot.SlotType, RoF2Slot.Unknown02, RoF2Slot.MainSlot, RoF2Slot.SubSlot, RoF2Slot.AugSlot, RoF2Slot.Unknown01, TitaniumSlot);

	return TitaniumSlot;
}

static inline uint32 MainInvRoF2ToTitaniumSlot(structs::MainInvItemSlotStruct RoF2Slot)
{
	uint32 TitaniumSlot = 0xffffffff;
	uint32 TempSlot = 0;

	if (RoF2Slot.MainSlot < 57)				// Worn/Personal Inventory and Cursor (Originally 33)
	{
		if (RoF2Slot.MainSlot == 21)
		{
			TempSlot = 9999;
		}
		else if (RoF2Slot.MainSlot >= 33)	// Cursor and Extended Corpse Inventory
		...
		/*else if (RoF2Slot.MainSlot == 31 || RoF2Slot.MainSlot == 32) { // 9th and 10th RoF inventory slots
			// Need to figure out what to do when we get these

			// Same as above
		}*/
		else if (RoF2Slot.MainSlot >= 22)	// Main Inventory and Ammo Slots
		{
			TempSlot = RoF2Slot.MainSlot - 1;
		}
		else
	...
	_log(NET__ERROR, "Convert RoF2 Slots: Main %i, Sub %i, Aug %i, Unk1 %i to Titanium Slot %i", RoF2Slot.MainSlot, RoF2Slot.SubSlot, RoF2Slot.AugSlot, RoF2Slot.Unknown01, TitaniumSlot);

	return TitaniumSlot;
}

// Converts Titanium Slot IDs to RoF2 Slot IDs for use in Encodes
static inline structs::MainInvItemSlotStruct MainInvTitaniumToRoF2Slot(uint32 TitaniumSlot)
{
	...
	if (TitaniumSlot < 56 || TitaniumSlot == 9999) // (Originally 52)
	{
		...
		/*else if (TitaniumSlot > 29)		// Cursor
		{
			...
			if (TitaniumSlot > 30)
			{
				RoF2Slot.SubSlot = (TitaniumSlot + 3) - 33;
			}
		}*/
	}
	else if (TitaniumSlot > 250 && TitaniumSlot < 341)
	...
	_log(NET__ERROR, "Convert Titanium Slot %i to RoF2 Slots: Main %i, Sub %i, Aug %i, Unk1 %i", TitaniumSlot, RoF2Slot.MainSlot, RoF2Slot.SubSlot, RoF2Slot.AugSlot, RoF2Slot.Unknown01);
...
ENCODE(OP_ShopPlayerSell) {
	...
	OUT(npcid);
	eq->itemslot = MainInvTitaniumToRoF2Slot(emu->itemslot);
	//OUT(itemslot);
	OUT(quantity);
	OUT(price);
	FINISH_ENCODE();
...
ENCODE(OP_ApplyPoison) {
	...
	eq->inventorySlot = MainInvTitaniumToRoF2Slot(emu->inventorySlot);
	OUT(success);
	FINISH_ENCODE();
}
...
ENCODE(OP_RecipeAutoCombine) {
	...
	OUT(object_type);
	OUT(some_id);
	eq->container_slot = TitaniumToRoF2Slot(emu->unknown1);
	structs::ItemSlotStruct RoF2Slot;
...
ENCODE(OP_DeleteItem) {
	...
	eq->from_slot = TitaniumToRoF2Slot(emu->from_slot);
	eq->to_slot = TitaniumToRoF2Slot(emu->to_slot);
	OUT(number_in_stack);
...
ENCODE(OP_ItemVerifyReply) {
	...
	eq->slot = TitaniumToRoF2Slot(emu->slot);
	OUT(spell);
	OUT(target);
...
ENCODE(OP_AdventureMerchantSell) {
	...
	eq->unknown000 = 1;
	OUT(npcid);
	eq->slot = MainInvTitaniumToRoF2Slot(emu->slot);
	OUT(charges);
	OUT(sell_price);
...
ENCODE(OP_CastSpell)
{
	...
	OUT(spell_id);
	eq->inventoryslot = TitaniumToRoF2Slot(emu->inventoryslot);
	//OUT(inventoryslot);
	OUT(target_id);
	FINISH_ENCODE();
}
...
ENCODE(OP_AltCurrencySell)
{
    ...
    eq->slot_id = TitaniumToRoF2Slot(emu->slot_id);
    OUT(charges);
    OUT(cost);
    FINISH_ENCODE();
}
...
DECODE(OP_AltCurrencySellSelection)
{
    ...
    emu->slot_id = RoF2ToTitaniumSlot(eq->slot_id);
    FINISH_DIRECT_DECODE();
}
...
DECODE(OP_AdventureMerchantSell) {
	...
	IN(npcid);
	emu->slot = MainInvRoF2ToTitaniumSlot(eq->slot);
	IN(charges);
	IN(sell_price);
...
DECODE(OP_ApplyPoison) {
	...
	emu->inventorySlot = MainInvRoF2ToTitaniumSlot(eq->inventorySlot);
	IN(success);
...
DECODE(OP_ItemVerifyRequest) {
	...
	emu->slot = RoF2ToTitaniumSlot(eq->slot);
	IN(target);
...
DECODE(OP_CastSpell) {
	...
	IN(spell_id);
	emu->inventoryslot = RoF2ToTitaniumSlot(eq->inventoryslot);
	//IN(inventoryslot);
	IN(target_id);
...
DECODE(OP_DeleteItem)
{
	...
	emu->from_slot = RoF2ToTitaniumSlot(eq->from_slot);
	emu->to_slot = RoF2ToTitaniumSlot(eq->to_slot);
	IN(number_in_stack);
...
DECODE(OP_ShopPlayerSell) {
	...
	IN(npcid);
	emu->itemslot = MainInvRoF2ToTitaniumSlot(eq->itemslot);
	//IN(itemslot);
	IN(quantity);
	IN(price);
...
DECODE(OP_TradeSkillCombine) {
	...
	int16 slot_id = RoF2ToTitaniumSlot(eq->container_slot);
	if (slot_id == 4000) {
...
DECODE(OP_RecipeAutoCombine) {
	...
	IN(object_type);
	IN(some_id);
	emu->unknown1 = RoF2ToTitaniumSlot(eq->container_slot);
	IN(recipe_id);
	IN(reply_code);
...
DECODE(OP_AugmentItem) {
	...
	emu->container_slot = RoF2ToTitaniumSlot(eq->container_slot);
...
char* SerializeItem(const ItemInst *inst, int16 slot_id_in, uint32 *length, uint8 depth) {
	...
	structs::ItemSlotStruct slot_id = TitaniumToRoF2Slot(slot_id_in);
